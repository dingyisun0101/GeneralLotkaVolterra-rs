//! Sequential Workflow-native spatial replicator example.
...
use general_lotka_volterra_rs::{SpatialReplicator, SpatialReplicatorConfig, TimeStep};
...
impl TaskInputs {
    ...
    fn initial_space(&self) -> Result<ArrayD<f64>, ndarray::ShapeError> {
        ...
        let values = self.initial_cell.repeat(cells);
        ArrayD::from_shape_vec(IxDyn(&shape), values)
    }
}

fn main() -> Result<(), Box<dyn Error>> {
    let project = load_project("spatial_replicator")?;
...
fn run_task(
    ...
    let config = SpatialReplicatorConfig::new(
        shape,
        inputs.growth,
        diffusion,
        inputs.cutoff,
