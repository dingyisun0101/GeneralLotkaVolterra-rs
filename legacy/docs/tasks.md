...
Cargo examples do not share a task-example helper module. Each example under
`examples/<name>/` is self-contained, keeps its settings in `constants.rs`, and
calls the top-level `solvers::solve` dispatcher directly from `main.rs`. It then
writes `TaskOutcome` metadata using the same metadata helpers as the task
runners.
