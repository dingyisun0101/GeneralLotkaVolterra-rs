...
fn python_decoder_fixture_is_exact_canonical_serde_output() {
    ...
    let abundance = Array1::from_vec(vec![0.2, 0.3, 0.5]);
    let space = ArrayD::from_shape_vec(IxDyn(&[1, 2, 2]), vec![0.2, 0.8, 0.3, 0.7]).unwrap();
    assert_eq!(
    ...
    assert_eq!(
        serde_json::to_value(Option::<ArrayD<f64>>::None).unwrap(),
