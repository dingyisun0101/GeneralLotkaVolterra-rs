...
For application code, prefer the task runners or the top-level
`solvers::solve` dispatcher. The lower-level solver functions are public so
advanced users can assemble custom workflows, but they expose more
implementation detail, including state ownership and save-cadence parameters.
The Cargo examples under `examples/<name>/` show direct dispatcher usage without
shared Rust example scaffolding.

## Unified Solve API

`solvers::solve` dispatches by `Dynamics`, `Space`, and `Noise`:
...
Use `Space::spatial(&diffusion, save_space_interval)` for spatial runs.
Supported combinations are well-mixed replicator, spatial replicator, and
spatial GLV population dynamics. Well-mixed GLV population dynamics returns an
`Unsupported` error until that integrator exists.
...
Noise is an optional post-step update shared by non-spatial and spatial
solvers:

- `NoiseKind::None`: no stochastic update.
- `NoiseKind::ProportionalGaussian`: multiplicative Gaussian perturbation with
  approximate local mass projection before sanitization.
- `NoiseKind::DemographicGaussian`: additive Gaussian perturbation scaled by
  `sqrt(x_i)`.

Noise kernels operate on one species vector at a time. The caller restores the
appropriate invariant boundary afterward:

- well-mixed replicator: `SystemState::sanitize`.
- spatial replicator: local per-cell simplex sanitization and aggregate refresh.
...
Spatial solver wrappers mirror the non-spatial termination split and accept the
same shared `Noise` model:
...
- `src/solvers/noise.rs`: shared noise configuration and raw slice kernels.
