/*!
Demographic-noise well-mixed replicator example.

Purpose:
    Demonstrates direct use of the unified solver API with a well-mixed
    frequency state and post-step demographic Gaussian noise.
*/
...
use general_lotka_volterra_rs::Mode;
...
use general_lotka_volterra_rs::utils::create_well_mixed_gs;
use ndarray::Array2;
...
fn main() -> Result<()> {
    ...
    let outcome = solve(
        ...
        SolveConfig {
            ...
            space: Space::None,
            // Demographic noise is applied after each deterministic RK4 step.
            // `SIGMA` controls the noise amplitude.
            noise: Noise::demographic_gaussian(SIGMA),
            dt: DT,
