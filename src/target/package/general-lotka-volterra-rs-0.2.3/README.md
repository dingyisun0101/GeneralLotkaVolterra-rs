...
The Rust solve finishes before plotting starts. Plot rendering uses the bundled
Python module `examples.plotting.render_from_output`, so activate a Python
environment with `numpy` and `matplotlib` before running examples end-to-end.
Without those packages, the simulation output and metadata may still be written,
but the example exits with a plot-rendering error.
