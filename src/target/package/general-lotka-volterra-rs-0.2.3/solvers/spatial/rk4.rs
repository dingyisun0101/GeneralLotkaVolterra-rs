...
//! spatial field, `space[x0, x1, ..., x{k-1}, i]`, with optional post-step
//! stochastic updates supplied by `spatial::noise`.
//!
...
///   - `layout`: Cached species-last shape/stride facts.
#[inline]
pub(super) fn sanitize_space_and_refresh_state(
...
///   - `layout`: Cached species-last shape/stride facts.
#[inline]
pub(super) fn sanitize_local_simplex_space_and_refresh_state(
    gs: &mut SystemState<f64>,
    layout: &SpatialLayout,
) -> Result<()> {
...
fn solve_impl(
    ...
    for step in 1..=num_steps {
        ...
        if let Some(checker) = termination_checker.as_mut()
            && let Some(reason) = checker.check(&gs_curr, step)
        {
...
mod tests {
    ...
    #[test]
    fn noisy_replicator_solver_keeps_each_spatial_cell_on_simplex() {
        let shape = vec![2, 2, 2];
        let data = vec![0.9, 0.1, 0.25, 0.75, 0.4, 0.6, 0.8, 0.2];
        let space = ArrayD::from_shape_vec(IxDyn(&shape), data).expect("valid shape");
        let gs = SystemState::from_arrays(
        ...
        let interaction_matrix = Array2::zeros((2, 2));
        let growth_vector = Array1::zeros(2);
        let diffusion = Diffusion::unit_spacing(Array1::zeros(2), 2, Boundary::Neumann);
        let output_path = temp_output_dir("replicator_noise_simplex");
        ...
        let out = solve_replicator(
            ...
            Some(&growth_vector),
            &diffusion,
            Noise::demographic_gaussian(1.0),
            0.01,
            3,
            3,
            3,
        ...
        for cell in 0..4 {
            let base = cell * 2;
            assert!(values[base].is_finite());
            assert!(values[base + 1].is_finite());
            assert!(values[base] >= 0.0);
            assert!(values[base + 1] >= 0.0);
            assert!((values[base] + values[base + 1] - 1.0).abs() < 1e-12);
        }
        assert!((out.state.sum() - 1.0).abs() < 1e-12);
        assert_eq!(out.mass, 1.0);
    ...
    #[test]
    fn noisy_spatial_glv_respects_population_feasibility() {
        let shape = vec![2, 2, 2];
        let space = ArrayD::from_elem(IxDyn(&shape), 1.0);
        let gs = SystemState::from_arrays(
            Mode::Population {
                cutoff: None,
                carrying_capacity: Some(4.0),
            },
            0,
            Array1::zeros(2),
            Some(space),
        ...
        let growth_vector = Array1::zeros(2);
        let diffusion = Diffusion::unit_spacing(Array1::zeros(2), 2, Boundary::Neumann);
        let output_path = temp_output_dir("glv_noise_feasibility");
        ...
        let final_space = out.space.expect("space retained");
        for &value in final_space.as_slice_memory_order().unwrap() {
        ...
        assert!(out.state.sum() <= 4.0 + 1e-12);
        let _ = fs::remove_dir_all(output_path);
    }
